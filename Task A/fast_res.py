...
#		print(logits)
		if phase == PredictionPhase.Training or PredictionPhase.Validating:
			loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.self_y, logits=logits)
...
def lexicon_recode(lex, words, labels, train=False):
	...
	if len(lex) == 13240:
		f = open("fast_train.txt", "r")
	else:
		f = open("fast_test.txt", "r")       
	fast  = f.readlines() 
	fasts = []
	fastb = []
	i=0
	for x in fast:
        	if (regex.match(x)):               
        	        	fastb.append(",".join(fasts))
        	        	fasts = []                        
        	else: 
                    a =  x.split()
                    if   reg.match(a[1]):                               
                    	fasts.append(a[1])                          
	for (line, tags), x in zip(lex.items(), fastb):
			y = x.split(",")
			for z in y:
				if reg.match(z):
