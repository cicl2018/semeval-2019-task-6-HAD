	...
	with open(filename, "r") as f:
		...
		for line in f:
			...
			if len(fields) > 1 :
				...
				lex[fields[0] + " " + pre_processing(fields[1])] = {"Task1":{fields[4].strip():1.0}}
		return lex
...
def model_training(configuration, train_batches, validation_batches, number):
	...
	with tf.Session() as sess:
		...
		for epoch in range(configuration.num_epochs):
			...
			with open ("IND.txt", "r") as off:
				IND_list = off.readlines()
			with open ("GRP.txt", "r") as off:
				GRP_list = off.readlines()
			with open ("oth.txt", "r") as off:
				oth_list = off.readlines()
			with open ("pronouns.txt", "r") as off:
				p_list = off.readlines()
			...
			with open (str(f1 * 100) + ".txt", "w+") as w:
					with open("numbers.txt", "r+") as f:
						...
						for item in range(len(all_text)):
							...
							for bad_word in IND_list:
								search = ""
								search = bad_word.replace("\n", "")
								if search in " ".join(tweets):
									if len(search) > 3:
										all_labels[item] = 3
										break

							for bad_word in GRP_list:
								search = ""
								search = bad_word.replace("\n", "")								
								if search in " ".join(tweets):
									if len(search) > 3:
							...
							if str(all_labels[item]) == "0":
								...
								w.write (" ".join(tweets) + " GRP\n")
							elif str(all_labels[item]) == "2":
								w.write (" ".join(tweets) + " OTH\n")
							elif str(all_labels[item]) == "3":
								w.write (" ".join(tweets) + " IND\n")
